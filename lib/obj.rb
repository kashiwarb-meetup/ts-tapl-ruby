...
# Extends basic.rb with: { x: 1, y: true } object literals and obj[:x] property access.
...
module Obj
  ...
  def self.parse_sig_type(str, _tvars = [])
    str = str.strip
  ...
  def self.typecheck(node, ty_env = {}, comments = [])
    ...
    in Prism::CallNode[name: :[], receiver:, arguments: Prism::ArgumentsNode[arguments: [Prism::SymbolNode[unescaped: prop_name]]]]
      ...
      obj_ty[:props].fetch(key) { raise TypeCheckError, "Property #{key} not found in #{obj_ty}" }
...
if __FILE__ == $0
  [
    ...
    ty = Obj.typecheck_source(src)
    puts "#{src.strip.split("\n").last.ljust(30)} => #{ty}"
  rescue Obj::TypeCheckError => e
